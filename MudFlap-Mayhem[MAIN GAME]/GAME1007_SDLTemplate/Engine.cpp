...
void Engine::Update()
{
	...
	//{
	//	/*if (m_player.GetPlayerAngle() == 90)
	//		m_pbullet[i]->UpdateRight();
	//	if (m_player.GetPlayerAngle() == 180)
	//		m_pbullet[i]->UpdateDown();
	//	if (m_player.GetPlayerAngle() == 0)
	//		m_pbullet[i]->UpdateUp();
	//	if (m_player.GetPlayerAngle() == 270)
	//		m_pbullet[i]->UpdateLeft();*/
	//}
...
void Engine::Render()
{
	...
	for (unsigned i = 0; i < m_pbullet.size(); i++)
		SDL_RenderCopyEx(m_pRenderer, m_pLaserTexture, nullptr, m_pbullet[i]->GetDst(),m_pbullet[i]->GetBulletAngle(),0,SDL_FLIP_NONE);
